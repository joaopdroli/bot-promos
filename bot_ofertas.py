...
# IDs corretos das categorias MLB
CATEGORIAS = {
...
def buscar_ofertas():
    ...
    for categoria_nome, categoria_id in CATEGORIAS.items():
        try:
            ...
            logging.info(f"Categoria {categoria_nome}: {len(items)} itens encontrados")
...
if __name__ == "__main__":
    ...
    job_queue.run_daily(postar_ofertas_automatico, time=time(11, 0))  # 8h BRT
    job_queue.run_daily(postar_ofertas_automatico, time=time(15, 0))  # 12h BRT
    job_queue.run_daily(postar_ofertas_automatico, time=time(20, 0))  # 17h BRT
    job_queue.run_daily(postar_ofertas_automatico, time=time(23, 0))  # 20h BRT
