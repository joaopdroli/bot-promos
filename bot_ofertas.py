...
def buscar_ofertas():
    ...
    logging.info(f"Total: {len(ofertas)} ofertas, com desconto: {sum(1 for o in ofertas if o['desconto'] > 0)}")
    return ofertas
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/forcarbusca — buscar e postar ofertas agora\n"
...
async def forcar_busca(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if postadas > 0:
        ...
        await update.message.reply_text("⚠️ Nenhuma oferta nova para postar no momento.")
...
async def receber_dados(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Calcula desconto se preço original fornecido
    if len(partes) > 2:
        try:
