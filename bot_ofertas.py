...
(AGUARDANDO_LINK, AGUARDANDO_DADOS, AGUARDANDO_CATEGORIA,
 AGUARDANDO_FRETE, AGUARDANDO_CUPOM, AGUARDANDO_CONFIRMACAO) = range(6)
...
async def receber_categoria(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ Sim, tem frete grátis", callback_data="frete_sim")],
            [InlineKeyboardButton("❌ Não", callback_data="frete_nao")],
        ])
    )
    return AGUARDANDO_FRETE

async def opcao_frete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    oferta_temp["frete"] = query.data == "frete_sim"
    await query.edit_message_text(
...
if __name__ == "__main__":
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            AGUARDANDO_FRETE: [CallbackQueryHandler(opcao_frete, pattern="^frete_")],
            AGUARDANDO_CUPOM: [
