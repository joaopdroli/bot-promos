...
def buscar_ofertas():
    ...
    for categoria_nome, categoria_id in CATEGORIAS.items():
        try:
            ...
            logging.info(f"{categoria_nome}: {len(items)} itens retornados")
    ...
    # Prioriza com desconto, mas posta qualquer produto
    ofertas.sort(key=lambda x: x["desconto"], reverse=True)
    logging.info(f"Total: {len(ofertas)} ofertas encontradas")
    return ofertas
