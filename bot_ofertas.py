...
BUSCAS = [
    ...
    "torneira cozinha banheiro",
    "luminária iluminação led",
    "tinta parede",
    "móveis sala escritório",
...
def buscar_ofertas():
    ...
    for busca in BUSCAS:
        try:
            ...
            logging.info(f"Busca '{busca}': {len(items)} itens")
            ...
            for item in items:
                ...
                if not titulo or not link or not preco:
                    continue

                if preco_original and isinstance(preco_original, (int, float)) and preco_original > preco:
                    desconto = int(((preco_original - preco) / preco_original) * 100)
                    preco_original_str = formatar_preco(preco_original)
                else:
                    desconto = 0
            ...
            logging.error(f"Erro busca '{busca}': {e}")

    ofertas.sort(key=lambda x: x["desconto"], reverse=True)
    logging.info(f"Total: {len(ofertas)} ofertas")
    return ofertas
...
# ===================== POSTAR =====================
async def _postar(bot, ofertas, n):
    postadas = 0
...
async def postar_ofertas_automatico(context):
    ...
    await _postar(context.bot, ofertas, OFERTAS_POR_CICLO)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/forcarbusca — buscar e postar agora\n"
        "/cancelar — cancelar operação"
    )

async def forcar_busca(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🔍 Buscando ofertas, aguarde...")
    ofertas = buscar_ofertas()
    if not ofertas:
        await update.message.reply_text("⚠️ API do ML não retornou produtos. Tente novamente.")
        return
    postadas = await _postar(context.bot, ofertas, OFERTAS_POR_CICLO)
    if postadas > 0:
        await update.message.reply_text(f"✅ {postadas} ofertas postadas no canal!")
    else:
        await update.message.reply_text("⚠️ Todos os produtos já foram postados hoje. Tente amanhã ou use /novaoferta.")
